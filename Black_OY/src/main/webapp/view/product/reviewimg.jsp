		...
		<div class="review_summary photo" id="photoDetail">
			<div class="user clrfix  ">
				...
				<p class="info_user">
					<a href="javascript:;"
						onclick="goods.gdas.handleClickReviewPhotoDetailReviewerProfile('aTJNaDhBM0V5QStaeWYrWXdXaWFPZz09', {t_page: '포토상세', t_click: '리뷰포토상세_리뷰어프로필', t_profile_name: 'Mizthecat', goodsNo: 'A000000190051', goodsname: '[윈터 PICK] 마몽드 포어 슈링커 바쿠치올 크림 60ml 단독기획 (+30ml 추가증정)', t_review_rank_name: '1172'});"
						class="id" data-attr="상품상세^포토상세^리뷰어프로필클릭">${reviewDTO.user_id }</a>
					<a href="javascript:;"
			...
			<div class="score_area addOY">
				<span class="review_point"><span class="point"
					style="width: ${reviewDTO.rev_grade * 20}%">5점만점에
						${reviewDTO.rev_grade}점</span></span> <span class="date">${reviewDTO.rev_reg}</span>
				<span class="ico_oyGroup">체험단</span>
			</div>
			<div class="txt_inner">${reviewDTO.rev_content}</div>
			...
			<div class="poll_sample">
				<dl class="poll_type1">
					...
					<dd>
						<span class="txt"> <c:choose>
								<c:when test="${reviewDTO.rev_grade_1 eq 3 }">
													건성에 좋아요
												</c:when>
								<c:when test="${reviewDTO.rev_grade_1 eq 2 }">
													복합성에 좋아요
												</c:when>
								<c:when test="${reviewDTO.rev_grade_1 eq 1 }">
													지성에 좋아요
												</c:when>
				...
				<dl class="poll_type1">
					...
					<dd>
						<span class="txt"> <c:choose>
								<c:when test="${reviewDTO.rev_grade_2 eq 3 }">
													보습에 좋아요
												</c:when>
								<c:when test="${reviewDTO.rev_grade_2 eq 2 }">
													진정에 좋아요
												</c:when>
								<c:when test="${reviewDTO.rev_grade_2 eq 1 }">
													주름/미백에 좋아요
												</c:when>
				...
				<dl class="poll_type1">
					...
					<dd>
						<span class="txt"> <c:choose>
								<c:when test="${reviewDTO.rev_grade_3 eq 3 }">
													자극없이 순해요
												</c:when>
								<c:when test="${reviewDTO.rev_grade_3 eq 2 }">
													보통이에요
												</c:when>
								<c:when test="${reviewDTO.rev_grade_3 eq 1 }">
													자극이 느껴져요
												</c:when>
