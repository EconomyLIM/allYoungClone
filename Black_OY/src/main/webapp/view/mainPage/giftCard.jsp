	...
	<div id="Container" class="main_container design-type-v1">
		...
		<div class="gc-content gc-landing">
			...
			<div class="theme-giftcard" data-category-name="NEW&amp;BEST">
				...
				<c:forEach items="${nlist }" var="list" >
					<div class="gc-card-info">
						<div>
							<span class="img"><img src="${list.gc_img_src}"></span>
							<p class="card-dic gc-f-mtg">${list.gc_name }</p>
						</div>
						<div class="card-btn">
							<button class="card-buy" type="button" onclick="getGiftCardForm('바로구매', '${list.giftcard_id}')">
								<span>바로구매</span>
							</button>
							<button class="card-gift" type="button" onclick="getGiftCardForm('선물하기', '${list.giftcard_id}')">
								<span>선물하기</span>
							</button>
						</div>
					</div>
			...
			<script>
					...
					function getGiftCardForm(type, giftcard_id) {
						let form = $("<form>").attr({
							method : "get"
							, action : "<%=contextPath%>/olive/giftCardForm.do"
						});
