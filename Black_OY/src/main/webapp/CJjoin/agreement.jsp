		...
		<script type="text/javascript">
							...
							success : function(objJson) {
								...
								if (objJson.correctBrandYn == "N") {
									...
									if (location.pathname
											.indexOf("/cjmweb/join") > -1
											|| location.pathname
													.indexOf("/cjmweb/member/find-id") > -1
											|| location.pathname
													.indexOf("/cjmweb/member/find-passwd") > -1
											|| location.pathname
													.indexOf("/cjmweb/member/change-passwd-step") > -1
											|| location.pathname
													.indexOf("/cjmweb/member/passwd") > -1
											|| location.pathname
													.indexOf("/cjmweb/member/account-lock/verify-account") > -1) {
										location.replace(location.pathname);
									} else if (location.pathname
											.indexOf("/cjmweb/member/terms-of-withdrawal") > -1) {
			...
			function search() {
				...
				if ($(".tSearch").hasClass("on")) {
					...
					location.href = "https://www.cjone.com/cjmweb/search.do?h_search_keyword="
							+ search_keyword;
				...
				if (loginCheck() == true) {
					if (isFirstDashOpen == false) {
						...
						$
								.ajax(
										{
											url : '/cjmweb/common/layer/dash-board-action.do?t='
													+ t
										})
								.done(
										function(html) {
											...
											$('[data-control="randomNum"]')
													.charMotion('runUp');
											$('[data-control="tooltip"]')
													.tooltip();
											$('[data-control="scrollView"]')
													.scrollview();
			...
			function notifyMessageAction(page_num) {
				...
				$.ajax({
					...
					success : function(response, statusText) {
						if (response.notifylist != undefined
								&& response.notifylist != null
								&& response.notifylist.length > 0) {
					...
					$(jsonList)
							.each(
									function(index, data) {
										...
										html += '<li>';
										if (data.read_yn == "Y") {
											html += '<a class="visited" href="javascript:goNotifyAction('
													+ data.notify_seq
													+ ','
													+ data.notify_link_url
													+ ')">';
										} else {
											html += '<a href="javascript:goNotifyAction('
													+ data.notify_seq
													+ ','
													+ data.notify_link_url
													+ ')">';
										...
										if (data.notify_section_name != ""
												&& data.notify_section_name != undefined) {
											html += '  <p>['
													+ data.notify_section_name
													+ '] ' + data.notify_msg
													+ '</p>';
										} else {
											html += '  <p>' + data.notify_msg
													+ '</p>';
										}
				...
				if (!loginCheck()) {
					if (confirm('카드 등록을 위해 로그인이 필요합니다.\n계속 진행하시겠습니까?')) {
						...
						$(location)
								.attr(
										'href',
										'/cjmweb/login.do?return_url='
												+ escape('https://www.cjone.com/cjmweb/my-one/card.do'));
						return false;
					}
				} else {
					location.href = 'https://www.cjone.com/cjmweb/my-one/card.do';
		...
		<div id="contentsWrap">
			...
			<form id="form1" name="form1" method="post" action="">
				...
				<input type="hidden" name="ipin_di" id="ipin_di" value=""> <input
					...
					type="hidden" name="agr_tm" id="agr_tm" value=""> <input
					type="hidden" name="agr_yn" id="agr_yn" value=""> <input
				...
				<input type="hidden" id="mbr_no" name="mbr_no" value=""> <input
					...
					type="hidden" id="mbr_typ_cd" name="mbr_typ_cd" value="11">
				<input type="hidden" id="join_coopco_list" name="join_coopco_list"
					value=""> <input type="hidden" id="coopco_hg_nm"
					name="coopco_hg_nm" value="올리브영"> <input type="hidden"
					name="agr_typ_onln" value=","> <input type="hidden"
					id="rejoin_yn" name="rejoin_yn" value="N"> <input
					type="hidden" id="nm_cnfm_yn" name="nm_cnfm_yn" value="Y">
				<input type="hidden" id="upd_coopco_id" name="upd_coopco_id"
					value=""> <input type="hidden" name="evt_typ" id="evt_typ"
					value=""> <input type="hidden" name="ret_mbr_id"
					id="ret_mbr_id" value=""> <input type="hidden"
					name="phone_no" id="phone_no" value="01092947310">
				...
				<input type="hidden" name="enc_data" id="enc_data"> <input
					...
					type="hidden" name="enc_com" id="enc_com"> <input
					type="hidden" id="legl_rep_nm" name="legl_rep_nm" value="">
				<input type="hidden" id="legl_birth_day" name="legl_birth_day"
					value=""> <input type="hidden" id="legl_ipin_ci"
					name="legl_ipin_ci" value=""> <input type="hidden"
					id="legl_cert" name="legl_cert" value=""> <input
					type="hidden" id="legl_typ_cd" name="legl_typ_cd" value="">
