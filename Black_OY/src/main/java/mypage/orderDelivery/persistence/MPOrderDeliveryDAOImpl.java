...
public class MPOrderDeliveryDAOImpl implements MPOrderDeliveryDAO {
	...
	public List<MPODdeliveryDTO> selectUODelivery(Connection conn, String oId) throws Exception {
		// TODO Auto-generated method stub
		String sql = " SELECT deli_recipient, d.delivery_id , deli_tel, deli_tel2, deli_zip, deli_road_addr, deli_addr, deli_baddr, o.delivery_msg msg "
		...
		if (rs.next()) {
			...
			do {
				...
				dto.setUodMsg(rs.getString("msg"));
				list.add(dto);
			} while (rs.next());
			JDBCUtil.close(pstmt);
			JDBCUtil.close(rs);
